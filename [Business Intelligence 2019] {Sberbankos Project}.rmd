...
authors: '{Anton Kasenkov,<br/> Qiuyue Chen,<br/> Irina Petrova}'
...
```{r}
### reading the data
data <- read.csv(file.choose()) # this is training data
str(data)
```
Now we have our training set. Preprocessing...
```{r}
finalTest <- read.csv(file.choose())# this is test data
...
```{r}
FGamma_score <- function(data, lev = NULL, model = NULL, anton = 2) {
        Recall1 <- sensitivity(data = data$pred, reference = data$obs,
                               positive = lev[1])
        Specificity1 <- specificity(data$pred, data$obs)
        Fgamma_score <- (1 + anton^2) * (Specificity1 * Recall1)/
                (anton^2 * Specificity1 + Recall1)
